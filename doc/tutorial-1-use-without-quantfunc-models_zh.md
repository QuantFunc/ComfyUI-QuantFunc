# 教程 1：不下载 QuantFunc 模型，直接使用 Workflow

[English Version](tutorial-1-use-without-quantfunc-models.md)

## 概述

你**不需要**下载 QuantFunc 预量化模型也能使用本插件。只要你有一个 **diffusers 格式**的基础模型（例如 [Qwen/Qwen-Image-Edit-2511](https://huggingface.co/Qwen/Qwen-Image-Edit-2511)），就可以通过 **Lighting 后端**实时量化并加速推理。

Lighting 后端会在加载时将 FP16 权重实时量化为低精度，无需预先转换。
...
点击 **Queue Prompt**。首次运行时 Lighting 引擎会对模型进行实时量化（需要额外几十秒），后续运行会使用缓存加速。
...
**Q: 首次加载很慢？**
A: 正常现象。Lighting 首次运行需要实时量化模型权重，后续运行会快很多。可以使用[教程 3](tutorial-3-export-custom-models_zh.md) 导出量化模型避免每次量化。

**Q: 哪些模型可以用？**
A: 任何 diffusers 格式的模型。目前支持的架构请查看 [QuantFunc 官方文档](https://www.modelscope.cn/models/QuantFunc)。

**Q: 和下载预量化模型比有什么区别？**
A: 预量化模型（SVDQ）加载更快、推理更快，因为量化过程在离线完成且使用了更先进的 SVD 量化算法。Lighting 从 FP16 实时量化更灵活但略慢。详见[教程 2](tutorial-2-download-and-use-quantfunc-models_zh.md)。
